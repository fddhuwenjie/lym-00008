...
class CDCLSolver:
    ...
    def _resolution(self, clause1: List[int], clause2: List[int], pivot: int) -> List[int]:
        lits1 = set(clause1)
        lits2 = set(clause2)
        lits1.discard(pivot)
        lits1.discard(-pivot)
    ...
    def _analyze_conflict(self, conflict_clause: List[int]) -> Tuple[List[int], int, List[List[int]]]:
        resolution_steps: List[List[int]] = []
        current_clause = list(conflict_clause)
        
        if self.enable_proof:
            resolution_steps.append(list(conflict_clause))
            ...
            for i in range(len(self.trail) - 1, -1, -1):
                ...
                if neg_lit in current_clause and get_level(lit) == self.decision_level:
                    ...
                    if reason is not None:
                        ...
                        if self.enable_proof:
                            resolution_steps.append(list(resolvent))
        ...
        if not levels:
            return learnt_clause, -1, resolution_steps
        ...
        if len(learnt_clause) == 1:
            ...
            other_levels = [get_level(l) for l in learnt_clause[:-1]]
            if other_levels:
                backtrack_level = max(other_levels)
            else:
                backtrack_level = 0
        
        return learnt_clause, backtrack_level, resolution_steps
    ...
    def _restart(self) -> None:
        ...
        if len(self.clauses) > len(self.original_clauses) + 2000:
            ...
            new_learnt_set = set()
            proof_keep = []
            for idx, c in enumerate(self.clauses[len(self.original_clauses):]):
                if len(c) <= 10:
                    key = str(sorted(c, key=lambda x: (abs(x), x)))
                    if key not in new_learnt_set:
                        new_learnt_set.add(key)
                        kept.append(c)
                        if self.enable_proof and idx < len(self.proof):
                            proof_keep.append(self.proof[idx])
    ...
    def solve(self, max_conflicts: int = 100000, max_time: float = 30.0) -> CDCLResult:
        ...
        while self.conflicts < max_conflicts:
            ...
            while conflict_loop_count < max_conflict_loops:
                ...
                learnt_clause, backtrack_level, resolution_steps = self._analyze_conflict(conflict)
                
                if self.enable_proof:
                    for step in resolution_steps:
                        if len(step) > 0 and step not in self.proof:
                            key = str(sorted(step, key=lambda x: (abs(x), x)))
                            if key not in self.learnt_set:
                                self.proof.append(list(step))
